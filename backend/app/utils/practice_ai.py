...
def _build_one_variant(
    ...
    target_count = int(parsed.get("target_count") or 8)
    selected: List[Question] = []
    type_counts = parsed.get("question_type_counts") or {}

    for question_type, count in type_counts.items():
        for _ in range(max(0, min(count, target_count))):
            picked = _pick_best(candidates, selected, score_map, variant_penalty, required_type=question_type)
            if not picked:
                break
...
def _serialize_variant(item: Dict[str, Any], parsed: Dict[str, Any], snapshot: Dict[str, Any]) -> AIPracticeVariant:
...
async def build_ai_preview(req: AIPracticePreviewRequest, user_id: int, db: Session) -> AIPracticePreviewResponse:
    parsed = await _parse_ai_requirement(req.prompt, user_id, db)
    ...
    variant_models = [_serialize_variant(item, parsed, snapshot) for item in variants]
